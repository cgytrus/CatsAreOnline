...
public static class Chat {
    ...
    private static readonly List<Message> messages = new(messagesCapacity);
    private static readonly List<string> history = new(historyCapacity);
    ...
    private static void MessageSent(string text) {
        ...
        if(_inputField) _inputField!.text = null;
        chatFocused = false;
    }
    ...
    public static void UpdateMessageHistory(bool up, bool down) {
        if(!chatFocused || !up && !down || !_inputField) return;
