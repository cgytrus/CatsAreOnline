...
public class StatusChangedMessageHandler {
    ...
    private void ConnectedMessageReceived(NetIncomingMessage message) {
        ...
        for(int i = 0; i < playerCount; i++) {
            ...
            string count = _playerRegistry.Count.ToString(CultureInfo.InvariantCulture);
            _playerRegistry.Add(player.username ?? count, player);
        }
        ...
        for(int i = 0; i < syncedObjectCount; i++) {
            ...
            if(syncedObject is null) continue;
            _syncedObjectRegistry.Add(syncedObject.id, syncedObject);
        }

        _client.AddCat();

        bool inCompanion = _client.companionId != Guid.Empty && _client.companionState is not null;
        if(inCompanion)
    ...
    private void DisconnectedMessageReceived(NetBuffer message) {
        ...
        if(MultiplayerPlugin.connected is not null) MultiplayerPlugin.connected.Value = false;
